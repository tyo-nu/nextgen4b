...
def filterSample(fName, peName, bcs, templates, f_filt_seqs, r_filt_seqs, saveIntermediate=False,
    ...
    for expt in bcs.keys():
        ...
        seqs = filterPEMismatches(bcSeqs[expt], peSeqs, copiedFunctionGenerator(fREs[0]))
...
def getCopiedSequence(s, fRE):
    return s[:fRE.search(str(s.seq)).start()]
    
def copiedFunctionGenerator(fRE):
    return lambda s: getCopiedSequence(s, fRE)
